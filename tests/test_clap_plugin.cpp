    ...
    clap_istream_t clap_stream() {
        ...
        s.read = [](const clap_istream_t* s, void* data, uint64_t n) -> int64_t {
            ...
            const std::size_t take =
                static_cast<std::size_t>(n) < avail ? static_cast<std::size_t>(n) : avail;
...
TEST_CASE("state: save succeeds and produces non-empty blob") {
    ...
    const auto* state =
        static_cast<const clap_plugin_state_t*>(p->get_extension(p, CLAP_EXT_STATE));
...
TEST_CASE("state: save + load round-trip preserves param values") {
    ...
    const auto* params =
        static_cast<const clap_plugin_params_t*>(p1->get_extension(p1, CLAP_EXT_PARAMS));
    const auto* state =
        static_cast<const clap_plugin_state_t*>(p1->get_extension(p1, CLAP_EXT_STATE));
    ...
    for (uint32_t i = 0; i < n; ++i) {
        ...
        if (std::strcmp(info.name, "tempo_hz") == 0)
            tempo_id = info.id;
        if (std::strcmp(info.name, "beat_phase") == 0)
            beat_id = info.id;
    }
    ...
    const auto* state2 =
        static_cast<const clap_plugin_state_t*>(p2->get_extension(p2, CLAP_EXT_STATE));
    const auto* params2 =
        static_cast<const clap_plugin_params_t*>(p2->get_extension(p2, CLAP_EXT_PARAMS));
...
TEST_CASE("state: load before activate restores values on subsequent activate") {
    ...
    for (uint32_t i = 0; i < params1->count(p1); ++i) {
        ...
        if (std::strcmp(info.name, "is_playing") == 0) {
            playing_id = info.id;
            break;
        }
    }
    ...
    out.ctx      = nullptr;
    out.try_push = [](const clap_output_events_t*, const clap_event_header_t*) -> bool {
        return true;
    };

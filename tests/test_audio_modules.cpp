...
TEST_CASE("AudioOutputModule: writes L and R inputs to buffer each sample") {
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        args.frame                                   = i;
        m.process(args);
        REQUIRE_THAT(L[static_cast<std::size_t>(i)], Catch::Matchers::WithinAbs(vals[i], 1e-6f));
        REQUIRE_THAT(R[static_cast<std::size_t>(i)], Catch::Matchers::WithinAbs(-vals[i], 1e-6f));
    }
}
...
TEST_CASE("AudioInput → AudioOutput: stereo pass-through via engine (1-sample latency)") {
    ...
    for (int i = 0; i < N; ++i) {
        src_L[i] = static_cast<float>(i + 1) * 0.1f;
        src_R[i] = -src_L[i];
    }

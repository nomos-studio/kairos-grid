...
TEST_CASE("EnvironmentModule: apply_params by port_schema name lookup") {
    ...
    for (const auto& e : res->port_schema().entries) {
        if (e.name == "env/tempo_hz")       frame[static_cast<std::size_t>(e.id)] = 3.0f; // 180 BPM
        if (e.name == "env/beat_phase")     frame[static_cast<std::size_t>(e.id)] = 0.1f;
        if (e.name == "env/is_playing")     frame[static_cast<std::size_t>(e.id)] = 1.0f;
        if (e.name == "env/voice_velocity") frame[static_cast<std::size_t>(e.id)] = 64.f;
    }

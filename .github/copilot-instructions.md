# nomos-studio C++ review instructions — kairos-grid (CLAP plugin engine)
...
## CLAP plugin — thread model (kairos-grid-specific)

kairos-grid is a CLAP **plugin**. The CLAP specification defines a strict
thread model that the host enforces. Flag any violation:

- **`process()`** is called on the audio thread. All audio-thread rules
  above apply without exception. No allocation, no blocking, no mutex.
- **`activate()` / `deactivate()`** are called on the main thread.
  Shared state between `activate()` and `process()` must go through
  `rcu_managed` or an SPSC queue — not a plain member variable.
- **Parameter changes** arrive via the `clap_process_t::in_events` queue
  during `process()` on the audio thread. Any state that is updated by
  a parameter change and also read by the DSP must be accessed via an
  approved lock-free mechanism.
- **`EnvironmentModule`** is the approved abstraction for sample-rate and
  block-size state shared between `activate()` and `process()`. Flag any
  code that reads `sample_rate` or `max_frames_count` outside of
  `EnvironmentModule` without equivalent synchronisation.

The Surge XT DSP wrappers (`KAIROS_GRID_BUILD_SURGE=ON`) run inside
`process()` on the audio thread. Surge XT allocates internally during
`init()` / `init_extra_config()` — those calls must not appear in the
`process()` path. Flag any Surge XT initialisation call that could be

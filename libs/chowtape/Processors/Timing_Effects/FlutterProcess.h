...
class FlutterProcess {
  public:
    ...
    inline bool shouldTurnOff() const noexcept {
        return depthSlew[0].getTargetValue() == depthSlewMin;
    }
    ...
    inline std::pair<float, float> getLFO(int n, size_t ch) noexcept {
        updatePhase(ch);
        flutterPtrs[ch][n] =
            depthSlew[ch].getNextValue() *
            (amp1 * std::cos(phase1[ch] + phaseOff1) + amp2 * std::cos(phase2[ch] + phaseOff2) +

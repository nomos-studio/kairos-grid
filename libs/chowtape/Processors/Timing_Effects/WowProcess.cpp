...
void WowProcess::prepareBlock(float curDepth, float wowFreq, float wowVar, float wowDrift,
...
void WowProcess::plotBuffer() {
    if (shouldTurnOff())
        wowBuffer.clear();
    wowBuffer.applyGain(0, wowBuffer.getNumSamples(), 0.83333f / amp);
}

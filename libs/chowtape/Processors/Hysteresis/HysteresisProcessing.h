...
#include "HysteresisSTN.h" // stub — RTNeural omitted; STN case falls through to RK4

enum SolverType { RK2 = 0, RK4, NR4, NR8, STN, NUM_SOLVERS };
...
class HysteresisProcessing {
  ...
  private:
    ...
    // STN solver: RTNeural omitted — fall back to RK4 quality
    template <typename Float> inline Float STNSolver(Float H, Float H_d) noexcept {
        return RK4Solver(H, H_d);
    }

...
#include "../../compat/chowdsp_compat.hpp"
#include <cmath>
...
/** Langevin function */
template <typename Float> static inline Float langevin(const HysteresisState& hp) noexcept {
#if HYSTERESIS_USE_SIMD
...
template <typename Float> static inline Float langevinD(const HysteresisState& hp) noexcept {
#if HYSTERESIS_USE_SIMD
...
template <typename Float> static inline Float langevinD2(const HysteresisState& hp) noexcept {
#if HYSTERESIS_USE_SIMD
    return xsimd::select(hp.nearZero, NEG_TWO_OVER_15 * hp.Q,
                         (Float)2.0 * hp.coth * (hp.cothSq - (Float)1.0) -
                             ((Float)2.0 * hp.oneOverQCubed));
#else

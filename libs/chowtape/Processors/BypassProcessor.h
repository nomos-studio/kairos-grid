...
class BypassProcessor {
  public:
    ...
    static bool toBool(const std::atomic<float>* param) { return static_cast<bool>(param->load()); }
    ...
    void processBlockOut(AudioBuffer<float>& block, bool onOffParam) {
        ...
        if (!bufferCopied)
            return; // parameter was changed in the middle of the buffer, let's wait for the next
                    // one!

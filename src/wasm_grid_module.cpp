...
namespace {
    ...
    static constexpr const char* k_faust_leaf_types[] = {"hslider", "vslider",  "nentry",
...
std::unique_ptr<WasmGridModule> WasmGridModule::create(const std::string& wasm_path) {
    ...
    auto* wm =
        new (std::nothrow) WasmGridModule(std::move(impl), n_audio_in, n_audio_out, n_params);
    if (!wm)
        return nullptr;

    // Populate param_ports with raw labels (stem prefix applied later by registry).
    for (int i = 0; i < n_params; ++i) {
        wm->param_ports.push_back({wm->impl_->params[static_cast<size_t>(i)].name, n_audio_in + i});
    }

    return std::unique_ptr<WasmGridModule>(wm);
}

WasmGridModule::WasmGridModule(std::unique_ptr<WasmDspImpl> impl, int n_audio_in, int n_audio_out,
                               int n_params)
    : GridModule(n_audio_in + n_params, n_audio_out), impl_(std::move(impl)),
      n_audio_in_(n_audio_in), n_audio_out_(n_audio_out), n_params_(n_params),
      last_param_voltages_(static_cast<size_t>(n_params), std::numeric_limits<float>::quiet_NaN()) {
}
...
void WasmGridModule::prepare(const GridProcessArgs& args) {
    ...
    wasmtime_val_t init_args[2] = {wt_i32(0), wt_i32(static_cast<int32_t>(args.sample_rate))};
    ...
    if (!impl_->scratch_ready) {
        ...
        wasmtime_error_t* err = wasmtime_memory_grow(ctx, &impl_->wasm_memory, 1, &prev_pages);
        if (err) {
            wasmtime_error_delete(err);
            return;
        }

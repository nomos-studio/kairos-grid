...
result<GridEngine, graph_error> GridGraph::build() {
    ...
    for (const auto& c : cables_) {
        if (c.from_module < 0 || c.from_module >= n || c.to_module < 0 || c.to_module >= n)
        ...
        if (c.from_port < 0 || c.from_port >= static_cast<int>(from_out.size()) || c.to_port < 0 ||
            c.to_port >= static_cast<int>(to_in.size()))

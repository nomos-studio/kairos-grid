...
static const clap_plugin_descriptor_t k_descriptor = {
    ...
    .version = "0.1.0",
    .description =
        "Bitwig Grid-style modular engine with nomos-rt control plane",
    .features = k_features,
};
...
struct ModuleSpec {
  ...
  std::function<std::unique_ptr<GridModule>(const std::string &)>
      make_with_args;
  std::function<void(GridModule *, const std::string &, const std::string &)>
      setup_with_args;
};

static const std::unordered_map<std::string, ModuleSpec> &
get_module_registry() {
  static const std::unordered_map<std::string, ModuleSpec> reg = []() {
    std::unordered_map<std::string, ModuleSpec> r;
    r["env"] = {[]() -> std::unique_ptr<GridModule> {
                  return std::make_unique<EnvironmentModule>();
                },
                nullptr, nullptr, nullptr};
    r["audio-in"] = {[]() -> std::unique_ptr<GridModule> {
                       return std::make_unique<AudioInputModule>();
                     },
                     nullptr, nullptr, nullptr};
    r["audio-out"] = {[]() -> std::unique_ptr<GridModule> {
                        return std::make_unique<AudioOutputModule>();
                      },
                      nullptr, nullptr, nullptr};
#if defined(KAIROS_GRID_PLUGIN_HAS_MI)
    r["plaits"] = {[]() -> std::unique_ptr<GridModule> {
                     return std::make_unique<mi::PlaitsModule>();
                   },
                   [](GridModule *m, const std::string &pfx) {
                     m->param_ports = {
                         {pfx + "/harmonics", 1}, {pfx + "/timbre", 2},
                         {pfx + "/morph", 3},     {pfx + "/engine", 6},
                         {pfx + "/level", 5},
                     };
                   },
                   nullptr, nullptr};
    r["svf"] = {[]() -> std::unique_ptr<GridModule> {
                  return std::make_unique<mi::SvfModule>();
                },
                [](GridModule *m, const std::string &pfx) {
                  m->param_ports = {
                      {pfx + "/cutoff", 1},
    ...
    r["wasm"] = {
        ...
        [](GridModule *m, const std::string & /*type*/,
...
static std::size_t find_section_start(const std::string &s, const char *key,
                                      char open) {
...
static std::string extract_type(const std::string &s, std::size_t start,
                                std::size_t end) {
  ...
  if (mstart != std::string::npos) {
    ...
    while (pos < s.size() && depth > 0) {
      ...
      if (c == '{') {
        ...
        if (!t.empty()) {
          const std::string wp =
              extract_str_field(s, ":wasm-path", map_start, pos);
...
private:
  ...
  void destroy() {
    PatchSlot *pending =
        pending_slot_.exchange(nullptr, std::memory_order_relaxed);
    delete pending;
    delete this;
  }
  ...
  bool activate(double sample_rate, uint32_t /*min_frames*/,
    ...
    const uint32_t new_size = static_cast<uint32_t>(
  ...
  void try_install_pending_slot() {
    PatchSlot *next =
        pending_slot_.exchange(nullptr, std::memory_order_acq_rel);
    if (!next)
      return;
    ...
    const uint32_t n = static_cast<uint32_t>(std::min(
        next->param_frame.size(), static_cast<std::size_t>(kMaxParams)));
  ...
  bool hot_swap_request_impl(const std::string &new_path,
    ...
    const std::string new_edn =
        current_edn_.substr(0, vs) + new_path + current_edn_.substr(ve);

    return push_patch_impl(new_edn.c_str(),
                           static_cast<uint32_t>(new_edn.size()));
  }
  ...
  static uint32_t audio_ports_count(const clap_plugin_t * /*p*/,
                                    bool /*is_input*/) {
    return 1;
  }
  ...
  static bool params_get_value(const clap_plugin_t *p, clap_id param_id,
                               double *out) {
    const auto *self = cast(p);
    ...
    *out = static_cast<double>(
        self->param_frame_[idx].load(std::memory_order_relaxed));
    return true;
  }
  ...
  static bool stream_write_all(const clap_ostream_t *s, const void *buf,
                               uint64_t n) {
  ...
  static bool state_save(const clap_plugin_t *p, const clap_ostream_t *s) {
    ...
    for (const auto &e : schema.entries) {
      ...
      const float value =
          (eid < self->param_count_.load(std::memory_order_relaxed))
  ...
  void rebuild_tap_schema_c() {
    ...
    tap_schema_c_.entries =
        tap_entries_c_.empty() ? nullptr : tap_entries_c_.data();
  }

  static const clap_kairos_tap_schema_t *
  tap_bus_get_schema(const clap_plugin_t *p) {
    return &cast(p)->tap_schema_c_;
  }

  static const float *tap_bus_get_tap_frame(const clap_plugin_t *p,
                                            uint32_t *out_count) {
    const auto *self = cast(p);
    *out_count = 0;
    if (!self->engine_.has_value())
  ...
  void rebuild_param_schema_c() {
    ...
    param_schema_c_.entries =
        param_entries_c_.empty() ? nullptr : param_entries_c_.data();
  }

  static const clap_kairos_param_schema_t *
  param_bus_get_schema(const clap_plugin_t *p) {
  ...
  static bool param_bus_set_param_frame(const clap_plugin_t *p,
    ...
    auto *self = cast_mut(p);
    const uint32_t n =
        std::min(count, self->param_count_.load(std::memory_order_relaxed));
    if (n == 0)
      return false;
    for (uint32_t i = 0; i < n; ++i)
  ...
  static bool patch_bus_push_patch(const clap_plugin_t *p, const char *edn,
                                   uint32_t len) {
  ...
  void build_engine(float sr) {
    ...
    g.add_cable(
        {in_idx, AudioInputModule::k_left, out_idx, AudioOutputModule::k_left});
    g.add_cable({in_idx, AudioInputModule::k_right, out_idx,
                 AudioOutputModule::k_right});
#endif
  ...
  void apply_transport(const clap_event_transport_t *t) {
    ...
    set_param(env_playing_id_,
              (t->flags & CLAP_TRANSPORT_IS_PLAYING) ? 1.f : 0.f);

    if (t->flags & CLAP_TRANSPORT_HAS_BEATS_TIMELINE) {
      const double pos =
          static_cast<double>(t->song_pos_beats) / CLAP_BEATTIME_FACTOR;
      set_param(env_beat_id_, static_cast<float>(pos - std::floor(pos)));

      if (t->flags & CLAP_TRANSPORT_HAS_TIME_SIGNATURE) {
        const double bar_start =
            static_cast<double>(t->bar_start) / CLAP_BEATTIME_FACTOR;
  ...
  static bool s_activate(const clap_plugin_t *p, double sr, uint32_t mn,
                         uint32_t mx) {
    return cast_mut(p)->activate(sr, mn, mx);
  }
  static void s_deactivate(const clap_plugin_t *p) {
    cast_mut(p)->deactivate();
  }
  static bool s_start_processing(const clap_plugin_t *p) {
    return cast_mut(p)->start_processing();
  }
  static void s_stop_processing(const clap_plugin_t *p) {
    cast_mut(p)->stop_processing();
  }
  static void s_reset(const clap_plugin_t *p) { cast_mut(p)->reset(); }
  static clap_process_status s_process(const clap_plugin_t *p,
  ...
  static void s_on_main_thread(const clap_plugin_t *p) {
    cast_mut(p)->on_main_thread();
  }
...
factory_get_descriptor(const clap_plugin_factory_t *, uint32_t index) {

...
namespace surge {
    ...
    template <int OscType> class SurgeOscModule : public GridModule {
      public:
        ...
        ~SurgeOscModule() override { destroy_osc(); }
        ...
        void process(const GridProcessArgs&) override {
            if (process_pos_ >= BLOCK_SIZE) {
                ...
                sst::basic_blocks::mechanics::copy_from_to<BLOCK_SIZE_OS>(surge_osc_->output,
                                                                          buf_L_);
                sst::basic_blocks::mechanics::copy_from_to<BLOCK_SIZE_OS>(surge_osc_->outputR,
                                                                          buf_R_);
